...
class SupabaseMediaStore implements MediaObjectStore {
  ...
  @override
  Future<bool> exists(String key) => _bucket.exists(_path(key));
  ...
  Future<void> upload(String key, File source, {required String sha256}) async {
    try {
      await _bucket.upload(
        _path(key),
        source,
        fileOptions: FileOptions(metadata: <String, dynamic>{'sha256': sha256}),
      );
  ...
  @override
  Future<List<int>> download(String key) => _bucket.download(_path(key));
}
