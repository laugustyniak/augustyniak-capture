...
Feature-first layout under `lib/features/<feature>/{domain,data,presentation}`. Seven features: `recordings`, `transcription`, `processing`, `enrichment`, `settings`, `logs`, `shortcuts`. No state-management or DI package — plain `ChangeNotifier` + constructor injection.
...
- **It only ever *fills* user-editable fields.** `title` is written only when blank, `category` only when null, and `tags` only when empty, so a name, category or tag set someone corrected survives a retry. `summary` remains pure derived output and a re-run refreshes it. Clearing a user-editable field is how you ask for it to be filled again on the next processing run.
...
**Change history** (`recordings/domain/recording_revision.dart`, `recordings/data/revisions_repository.dart`) — what a model or a hand edit *overwrote*. It exists because the enrichment stage rewrites `title`, `category`, `summary` and `tags` on every re-run and `summary`/`tags` have no editor, so the previous answer was simply gone.
...
- UI: a collapsed `HISTORY` section at the bottom of the edit sheet — inline, no dialog, and the `CopyButton` on each entry hands back the **previous** value in full, which is how an overwritten transcript is recovered.
...
- `settings.json` (`settings/data/settings_repository.dart`) — provider profiles, the two active profile ids (transcription and enrichment), audio params, global shortcut bindings.
...
- `saveAll` copies the index to `recordings.shrank-<ts>.json` when the row count is **about to drop**. There is no delete in this app, so a shrinking index is by definition an anomaly; the check costs nothing in normal use because it never fires. `_knownCount` exists only for this and is the sole state in the repository.
...
- **Never `pumpAndSettle` a screen containing a `PulseDot` or a `ScanLine`** (a transcribing card, a card being enriched, the capture screen). Both repeat forever, so "no frames scheduled" is a state that screen never reaches and the call hangs until the timeout. Pump explicit frames instead.
