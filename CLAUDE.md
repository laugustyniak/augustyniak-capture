...
- **It only ever *fills* the two user-editable fields.** `title` is written only when blank and `category` only when null, so a name someone typed or a category someone corrected survives a retry. `summary` and `tags` have no editor, so they are pure derived output and a re-run refreshes them. Clearing the category (the `—` entry in the edit sheet) is therefore how you ask for a re-classification.
